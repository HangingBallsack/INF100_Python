...
class Board(Canvas):
    ...
    def __init__(self, level): # Python class initiation function
        super().__init__(width=BOARD_WIDTH, height=BOARD_HEIGHT, background="black", highlightthickness=0)
        self.initGame(level)
        self.pack()
    ...
    def initGame(self, level):
        ...
        self.moveX = 0 # relative x coordinate
        self.moveY = 0 # relative y coordinate
        # load map
        ...
        # Debug specter mode
        self.specterMode = False # press s to change to True
    ...
    def createObjects(self, game_map):
        ...
        self.create_text(30, 30, text="Life: {0}".format(self.life), tag="life", fill="white")
        # Create map
        for i in game_map: 
            map_type = game_map[i]
            ...
            if map_type == "M": # special case to hide jumping mario under mario
                self.create_image(xCoord, yCoord, image=self.jmario, anchor=NW, tag="jmario", state="hidden")
    ...
    def getCurrentMario(self):
        ...
        return(self.find_withtag("mario"))
...
if __name__ == '__main__' and len(PICTURES) > 0:
    main()
else:
